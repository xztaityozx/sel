...
- **Zero-copy line reading**: columns are `[]byte` all the way from `bufio`'s read buffer to `output.Writer`, so on the default lazy literal-separator path an index query allocates nothing per line — whatever the column count — and no `unsafe` is needed (`-S`/`--csv`/`--tsv` re-split every line, and `RegexpIterator` allocates on negative indices). Everything a `Columns` returns points into that buffer and is only valid until the next `Source.Next()` — copy with `bytes.Clone` to keep it longer
